...
@Component({
    ...
    <div class="java-list-page">
      ...
      <div class="search-wrapper">
        <input
          type="text"
          class="search-input"
          placeholder="Search Java & Spring topics..."
          [value]="searchQuery()"
        ...
        <section class="category-section">
          ...
          <div class="grid-4">
            @for (topic of group.topics; track topic.id) {
              <a [routerLink]="'/java/' + topic.id" class="card topic-card">
                <div class="topic-top">
  ...
  `,
  styles: [`
    .java-list-page { max-width: 1200px; }
    .page-subtitle {
      font-size: 0.9rem;
...
export class JavaListComponent implements OnInit {
  ...
  private categoryMeta: Record<JavaCategory, { label: string; icon: string }> = {
    'core-java': { label: 'Core Java', icon: '\u2615' },
    'spring-boot': { label: 'Spring Boot', icon: '\uD83C\uDF43' },
    'spring-security': { label: 'Spring Security', icon: '\uD83D\uDD12' },
    'spring-data': { label: 'Spring Data', icon: '\uD83D\uDDC4' },
    'spring-cloud': { label: 'Spring Cloud', icon: '\u2601' },
    'api-docs': { label: 'API Docs', icon: '\uD83D\uDCDC' },
    'rate-limiting': { label: 'Rate Limiting', icon: '\u23F1' },
    'validation': { label: 'Validation', icon: '\u2713' },
    'tooling': { label: 'Tooling', icon: '\uD83D\uDD27' }
  };

  private categoryOrder: JavaCategory[] = [
    'core-java', 'spring-boot', 'spring-security', 'spring-data',
    'spring-cloud', 'api-docs', 'rate-limiting', 'validation', 'tooling'
  ];
  ...
  getStatus(topicId: string): string {
    return this.progressService.getItemStatus('java', topicId, '_topic');
  }
  ...
  private initializeProgress(): void {
    ...
    this.progressService.initializeItems('java', items);
  }

  private updateOverallProgress(): void {
    let completed = 0;
    for (const topic of this.allTopics) {
      if (this.progressService.getItemStatus('java', topic.id, '_topic') === 'completed') {
        completed++;
      }
    }
