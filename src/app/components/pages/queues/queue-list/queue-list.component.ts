...
@Component({
        ...
        <section class="category-section">
          ...
          <div class="grid-4">
            @for (topic of group.topics; track topic.id) {
              <a [routerLink]="'/queues/' + topic.id" class="card topic-card">
                <div class="topic-top">
  ...
  `,
  styles: [`
    .queue-list-page { max-width: 1200px; }
    .page-subtitle {
      font-size: 0.9rem;
...
export class QueueListComponent implements OnInit {
  ...
  private categoryMeta: Record<QueueCategory, { label: string; icon: string }> = {
    'fundamentals': { label: 'Fundamentals', icon: '📨' },
    'kafka': { label: 'Kafka Deep Dive', icon: '📡' },
    'patterns': { label: 'Patterns', icon: '🔁' },
    'production': { label: 'Production', icon: '🏭' },
    'alternatives': { label: 'Alternatives', icon: '⚖' }
  };

  private categoryOrder: QueueCategory[] = ['fundamentals', 'kafka', 'patterns', 'production', 'alternatives'];
  ...
  getStatus(topicId: string): string {
    return this.progressService.getItemStatus('queues', topicId, '_topic');
  }
  ...
  private initializeProgress(): void {
    ...
    this.progressService.initializeItems('queues', items);
  }

  private updateOverallProgress(): void {
    let completed = 0;
    for (const topic of this.allTopics) {
      if (this.progressService.getItemStatus('queues', topic.id, '_topic') === 'completed') {
        completed++;
      }
    }
