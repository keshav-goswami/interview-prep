...
@Component({
  selector: 'app-golang-topic',
  standalone: true,
  imports: [RouterLink, StatusBadgeComponent, ExternalLinkComponent],
  template: `
    @if (topic()) {
      <div class="golang-topic-page">
        <!-- Navigation -->
        <div class="top-nav">
          <a routerLink="/golang" class="back-link">Back to Golang</a>
          <div class="prev-next">
            @if (prevTopic()) {
              <a [routerLink]="'/golang/' + prevTopic()!.id" class="nav-link">Prev</a>
            }
            @if (nextTopic()) {
              <a [routerLink]="'/golang/' + nextTopic()!.id" class="nav-link">Next</a>
            }
          </div>
        </div>
        ...
        <div class="bottom-nav">
          ...
          <div class="prev-next">
            @if (prevTopic()) {
              <a [routerLink]="'/golang/' + prevTopic()!.id" class="nav-link">Prev: {{ prevTopic()!.title }}</a>
            }
            @if (nextTopic()) {
              <a [routerLink]="'/golang/' + nextTopic()!.id" class="nav-link">Next: {{ nextTopic()!.title }}</a>
            }
          </div>
        </div>
      </div>
  ...
  `,
  styles: [`
    .golang-topic-page { max-width: 860px; }
    .top-nav, .bottom-nav {
      display: flex;
...
export class GolangTopicComponent implements OnInit {
  private route = inject(ActivatedRoute);
  private progressService = inject(ProgressService);

  topic = signal<GolangTopic | null>(null);
  prevTopic = signal<GolangTopic | null>(null);
  nextTopic = signal<GolangTopic | null>(null);
  topicStatus = signal<ItemStatus>('not-started');

  private allTopics = GOLANG_TOPICS;

  private categoryLabels: Record<GolangCategory, string> = {
  ...
  cycleStatus(): void {
    const t = this.topic();
    if (!t) return;
    this.progressService.cycleItemStatus('golang', t.id, '_topic');
    this.topicStatus.set(this.progressService.getItemStatus('golang', t.id, '_topic'));
  }

  categoryLabel(cat: GolangCategory): string {
  ...
  private loadTopic(topicId: string): void {
    ...
    this.topicStatus.set(this.progressService.getItemStatus('golang', topicId, '_topic'));
  }
}
