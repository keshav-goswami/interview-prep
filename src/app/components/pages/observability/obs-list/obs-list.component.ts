...
@Component({
    ...
    <div class="obs-list-page">
      ...
      <div class="search-wrapper">
        <input
          type="text"
          class="search-input"
          placeholder="Search observability topics..."
          [value]="searchQuery()"
        ...
        <section class="category-section">
          ...
          <div class="grid-4">
            @for (topic of group.topics; track topic.id) {
              <a [routerLink]="'/observability/' + topic.id" class="card topic-card">
                <div class="topic-top">
  ...
  `,
  styles: [`
    .obs-list-page { max-width: 1200px; }
    .page-subtitle {
      font-size: 0.9rem;
...
export class ObsListComponent implements OnInit {
  ...
  allTopics = OBSERVABILITY_TOPICS;
  ...
  private categoryOrder: ObsCategory[] = ['metrics', 'logging', 'tracing', 'performance', 'practices'];
  ...
  getStatus(topicId: string): string {
    return this.progressService.getItemStatus('observability', topicId, '_topic');
  }
  ...
  private initializeProgress(): void {
    ...
    this.progressService.initializeItems('observability', items);
  }

  private updateOverallProgress(): void {
    let completed = 0;
    for (const topic of this.allTopics) {
      if (this.progressService.getItemStatus('observability', topic.id, '_topic') === 'completed') {
        completed++;
      }
    }
